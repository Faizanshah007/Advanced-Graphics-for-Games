...
Renderer::Renderer(Window& parent) : OGLRenderer(parent) {
	...
	texture[0] = SOIL_load_OGL_texture(TEXTUREDIR"stainedglass.tga", 
		SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, 0);
	texture[1] = SOIL_load_OGL_texture(TEXTUREDIR"stainedglass.tga",
		SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, 0);
	texture[2] = SOIL_load_OGL_texture(TEXTUREDIR"stainedglass.tga",
		SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, 0);
	texture[3] = SOIL_load_OGL_texture(TEXTUREDIR"stainedglass.tga",
		SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, 0);
	texture[4] = SOIL_load_OGL_texture(TEXTUREDIR"stainedglass.tga",
		SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, 0);
...
void   Renderer::DrawNode(SceneNode* n) {
	if (n->GetMesh()) {
		...
		texture[0] = n->GetTexture();
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, texture[0]);

		glUniform1i(glGetUniformLocation(shader->GetProgram(), "useTexture"), texture[0]);
		n->Draw(*this);
	}
}
